...
ClassImp(AliHFJetContainer)

//const Int_t AliHFJetContainer::fgkCFSteps = 6;
//const Int_t AliHFJetContainer::fgkCFVars = 4;

AliHFJetContainer::AliHFJetContainer(const char* name): 
  TNamed(name,name),
  fCustomVarNames(0x0),
  fContainer(0)
{
...
AliHFJetContainer::AliHFJetContainer(const char* name, const Int_t nvars, const char* varnames[], Int_t *nbins, Double_t *binning[], const char*  axistitle[]): 
  TNamed(name,name),
  fCustomVarNames(0x0),
  fContainer(0)
{
  ...
  for (Int_t i=0; i<fgkCFVars; i++){
    ...
    AliInfo(Form(mag"Standard vars: ID \"%d\" NAME \"%s\" TITLE \"%s\""B, i, GetVarName((CFVars)i), totaxistitle[i])) ;
  }
  ...
  for (Int_t j=fgkCFVars; j<totnvars; j++){
    ...
    AliInfo(Form(cy"Custom vars: ID \"%d\" NAME \"%s\" TITLE \"%s\""B, j, GetVarName((CFVars)j), totaxistitle[j]));
  }
...
//----------------------------------------------------------------
AliHFJetContainer::AliHFJetContainer(const AliHFJetContainer &c) :
  TNamed(),
  fContainer(0)
{

  // AliHFJetContainer copy constructor

  ((AliHFJetContainer &) c).Copy(*this);
}

//----------------------------------------------------------------
AliHFJetContainer::~AliHFJetContainer()
{
  // Destructor
...
//----------------------------------------------------------------
AliHFJetContainer &AliHFJetContainer::operator=(const AliHFJetContainer &c)
{
  // assigment operator
...
//----------------------------------------------------------------
void AliHFJetContainer::Copy(TObject& c) const
{
  // copy function

  AliHFJetContainer& target = (AliHFJetContainer &) c;
...
//----------------------------------------------------------------
void AliHFJetContainer::CreateContainer(TString name, TString title, Int_t nvars, Int_t *nbins, Double_t *binning[], const char *axistitle[])
{
...
//----------------------------------------------------------------
Double_t* AliHFJetContainer::GetBinning(TString var, Int_t& nBins, const char*& axistitle)
{
...
void AliHFJetContainer::SetAxisRangeStep(const char* axisname, Double_t min, Double_t max, CFSteps step)
{
...
void AliHFJetContainer::SetAxisRangeAllSteps(const char* axisname, Double_t min, Double_t max)
{
...
void AliHFJetContainer::PrintVars()
{
...
void AliHFJetContainer::PrintSteps()
{
  Int_t nstep=fContainer->GetNStep();
  for (Int_t i=0; i<nstep; i++){
     Printf(mag"Step %d: %s -> %s"B,i, GetStepName((CFSteps) i), GetStepTitle((CFSteps) i));
  }
}

void AliHFJetContainer::ResetAxisStep(const char* axisname, CFSteps step)
{
...
void AliHFJetContainer::ResetAxisAllSteps(const char* axisname)
{
...
TH1D *AliHFJetContainer::Project1D(CFSteps step, const char* varname)
{
...
TH2D *AliHFJetContainer::Project2D(CFSteps step, const char* varname1, const char* varname2)
{
...
void AliHFJetContainer::ScaleStep(Double_t factor, CFSteps step)
{
...
void AliHFJetContainer::ScaleAllSteps(Double_t factor)
{
  for (Int_t i=0; i<fgkCFSteps; i++){
...
void AliHFJetContainer::SetStepNames(AliCFContainer* container)
{
...
const char* AliHFJetContainer::GetStepTitle(CFSteps step)
{
...
const char* AliHFJetContainer::GetStepName(CFSteps step)
{
  switch (step){
    case kCFStepAll:
      return "kCFStepAll";
...
const char* AliHFJetContainer::GetVarName(CFVars var)
{ 
...
Int_t AliHFJetContainer::GetVarAxis(const char* varname){
...
void AliHFJetContainer::CreateDefaultBinning()
{
...
TH1* AliHFJetContainer::StepsRatio(CFSteps num, CFSteps denom, Int_t var1, Int_t var2)
{
  TH1 *hnum=0;
  TH1 *hdenom=0;
  if (var2 >= 0){
...
TH1D* AliHFJetContainer::GetEfficiencyPt()
{
...
TH1D* AliHFJetContainer::GetEfficiencyEta()
{
...
TH2D* AliHFJetContainer::GetEfficiency2D()
{
...
TH1D* AliHFJetContainer::GetPurityPt()
{
...
TH1D* AliHFJetContainer::GetPurityEta()
{
...
TH2D* AliHFJetContainer::GetPurity2D()
{
