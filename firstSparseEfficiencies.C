...
void firstSparseEfficiencies(Float_t mass=0.,Int_t partonmethod=2, Float_t pt=10. ){
  ...
  //
  TH1D* hallb=(TH1D*)makeProjectionpt("hallb",fSparseRecoJets,axispart,5,0);
  TH1D* hallc=(TH1D*)makeProjectionpt("hallc",fSparseRecoJets,axispart,4,0);
  TH1D* hallg=(TH1D*)makeProjectionpt("hallg",fSparseRecoJets,axispart,2,0);
 
  if(partonmethod!=2)  TH1D* halllq=(TH1D*)makeProjectionpt("halllq",fSparseRecoJets,axispart,3,0);
  else  TH1D* halllq=(TH1D*)makeProjectionpt("halllq",fSparseRecoJets,axispart,1,0);
  ...
  if(partonmethod!=0 && partonmethod!=1 && partonmethod!=2) cout<<"???????????? WRONG PARTON AXIS SET ?????????????"<<endl;
